...
def out_b():
    data = {"status": True}
    requests.post(
        "http://0.0.0.0:5068/alarm",
        data=json.dumps(data),
        headers=CONTENT_HEADER,
    ) 


def analys(data, details):
    print(data['value'])
    current = data['value']
    details["Alarm"] = []
    if current > 18:
        details["Alarm"].append(
            {
                "event_id": details["id"],
                "event": "alarm",
                "curent_value": current
            }
        )
...
def handle_event(id: str, details: dict):    
    ...
    try:
        if details['operation'] == 'new_input':
            new_data = details["new_data"]
            analys(new_data, details)
            if details['Status'] == 'alarm':
                out_b()
            details["deliver_to"] = "gen_event"
            details["operation"] = "event_security"
